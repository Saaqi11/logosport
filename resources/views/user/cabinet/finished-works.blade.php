...
@section("content")
    ...
    <div class="row" id="cabinet-portal">
        @forelse ($works as $work)
            @if($work->finishContest)
                <div class="col-lg-3 col-md-6 col-sm-12 cabinet-slider-images-view" data-id={{ $work->finishContest->designerWork->id ?? '' }}>
                    <div class="prf-block prf-block--win">
                        <div class="prf-image">
                            <div class="count-img count-img--3">
                                @if (!empty($work->finishContest->designerWork) && !empty($work->finishContest->designerWork->files))
                                    <div class="wrp-few">
                                        <div class="first-image">
                                            <a href="#">
                                                <img class="image-cover" src="{{ asset(@$work->finishContest->designerWork->files[0]->src ?? "/images/ex-1.png") }}"  alt="">
                                            </a>
                                        </div>
                                        <div class="second-image">
                                            <a href="#">
                                                <img class="image-cover" src="{{ asset(@$work->finishContest->designerWork->files[1]->src ?? "/images/ex-2.png" ) }}" alt="">
                                            </a>
                                        </div>
                                    </div>
                                    <div class="third-image">
                                        <a href="#">
                                            <img class="image-cover" src="{{ asset(@$work->finishContest->designerWork->files[2]->src ?? "/images/ex-3.png")  }}" alt="">
                                        </a>
                                    </div>
                                @else
                        ...
                        <div class="prf-title">
                            <a href="{{ route('competition.show', [$work->finishContest->id])}}">
                                {{ $work->finishContest->company_name }}
                            </a>
                        </div>
                    </div>
                </div>
